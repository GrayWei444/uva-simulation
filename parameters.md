# UVA Lettuce Model - Parameter Documentation

**Model:** Lettuce Growth and UVA Effect Integrated Model
**Framework:** Carbon Competition + AOX/Anthocyanin

---

## Core Innovation

This model introduces **real carbon competition** between growth and defense:

1. **State variable:** AOX (Total Antioxidants)
2. **Output:** Anthocyanin = AOX x 18%
...
| K_uva_sla | 7.5 | W/m2 | SLA enhancement half-saturation |
| uva_lai_boost | 1.70 | - | UVA max enhancement on LAI growth |
| K_uva_lai | 7.5 | W/m2 | LAI enhancement half-saturation |

### 3. ROS Dynamics

| Parameter | Value | Unit | Description |
|-----------|-------|------|-------------|
| k_ros_production | 0.010 | ROS/(W/m2*s) | ROS production coefficient |
...
| k_vulnerability | 2.0 | 1/(m2/m2) | Vulnerability decay coefficient |
...
| K_aox_protection | 2.78e-5 | kg/m2 | AOX protection half-saturation |
...
| Parameter | Value | Unit | Description |
|-----------|-------|------|-------------|
| base_aox_rate_light | 3.53e-9 | kg/m2/s | Base synthesis (day) |
| base_aox_rate_dark | 1.77e-9 | kg/m2/s | Base synthesis (night) |
| V_max_aox | 1.45e-8 | kg/m2/s | Max stress-induced synthesis |
...
| carbon_competition_K | 1e-8 | kg/m2/s | Half-saturation for AOX carbon effect |
...
```
stress_carbon_effect = 0.225 x Stress / (21 + Stress)
aox_carbon_effect = stress_aox_carbon_demand / (1e-8 + stress_aox_carbon_demand)
carbon_competition_effect = aox_carbon_effect x 0.30 + stress_carbon_effect
growth_penalty = 1.0 - carbon_competition_effect
```

**Real C_buf Consumption:**
```
max_consumption = C_buf x 0.10  (10% per timestep)
...
| Parameter | Value | Unit | Description |
|-----------|-------|------|-------------|
| k_uv_aox | 7.78e-11 | kg/m2/s | UV induction coefficient |
...
| Parameter | Value | Unit | Description |
|-----------|-------|------|-------------|
| LAI_healthy | 9.0 | m2/m2 | Reference LAI for healthy plant |
...
| Parameter | Value | Unit | Description |
|-----------|-------|------|-------------|
| anthocyanin_fraction | 0.18 | - | Anthocyanin = AOX x 18% |
...
```
factor = 1 + 250 x exp(-exp(-0.5 x (hours - 10.5)))
```
...
### AOX Synthesis
```
stress_induced = V_max x Stress / (K + Stress) x night_eff x LAI_eff
uv_induced = k_uv_aox x total_hours / (K_uv_hours + total_hours)
aox_synthesis = LAI x (base + uv_induced + stress_induced x adaptation x nonlin_eff)
                x stress_eff x water_eff
```
...
total_uva_hours = max(0, days_irradiated - 1) x daily_hours + hours_today
```

### Carbon Competition Growth Penalty
```
growth_penalty = 1.0 - (aox_carbon_effect x carbon_competition_max + stress_competition_max x Stress/(stress_competition_K + Stress))
...
|  | (SLA, LAI boost)     |    | dROS/dt = k x I_UVA  |            |
|  +----------------------+    +----------------------+            |
+-----------------------------+------------------------------------+
                              |
                              v
+------------------------------------------------------------------+
|                    Stress Dynamics                                |
|  Vulnerability = A x exp(-k x LAI) + 1                           |
|  Damage = stress_coeff x ROS x vulnerability x (1 - AOX_prot)    |
...
+------------------------------------------------------------------+
|                    AOX Dynamics                                   |
|  synthesis = LAI x (base + UV + stress_induced) x efficiencies   |
...
+------------------------------------------------------------------+
|                    Output Conversion                              |
|  Anthocyanin = AOX x 0.18                                        |
|  FW = X_d / (DW:FW ratio)                                        |
|  Anth_ppm = Anthocyanin / FW x 1e6                               |
+------------------------------------------------------------------+
```
...
2. **Carbon consumption is realistic** - Uses `C_buf x 0.10` (10%) per timestep maximum.

3. **AOX scaling** - Since Anth = AOX x 0.18, all AOX rates are scaled by 1/0.18 = 5.56x from original Anth rates.

4. **All parameters in ALL_PARAMS** - Previously hardcoded constants (LAI efficiency, night efficiency, UV induction, ROS consumption, LDMC acute) are now configurable.
