...
def test(params):
    ...
    for treat in ['CK', 'L6D6', 'L6D6-N', 'VL3D12', 'L6D12', 'H12D3']:
        ...
        if sol.success:
            ...
            results[treat] = (FW, Anth_ppm, Stress)
    return results
...
# 目前問題:
# CK: +8.5% (base rate 過高)
# VL3D12: +9.4% (Stress 誘導過高或 K 太低)

# 分析 CK
print("\nCK 目標: 43.3 ppm, 目前: 47.0 ppm (+8.5%)")
print("CK 沒有 UVA，只有基礎合成，需要降低 base_anth_rate")
print()

# 計算需要的 base rate 調整
# 目前 47.0, 目標 43.3, 比例 = 43.3/47.0 = 0.921
# 需要降低約 8%

print("搜索 base_anth_rate_light:")
for base in [1.4e-10, 1.5e-10, 1.6e-10, 1.7e-10, 1.8e-10]:
    params = BASE.copy()
    params['base_anth_rate_light'] = base
    params['base_anth_rate_dark'] = base / 2
    results = test(params)
    ck_anth = results['CK'][1]
    ck_err = (ck_anth - 43.3) / 43.3 * 100
    print(f"  base={base:.1e}: CK={ck_anth:.1f} ({ck_err:+.1f}%)")

# 找到降低 CK 的 base rate 後，再調整 V_max 和 K
print("\n" + "=" * 80)
print("組合搜索 (base, V_max, K_stress_anth):")
print("=" * 80)

best = None
best_max_err = 100

for base in [1.4e-10, 1.5e-10, 1.6e-10]:
    for v_max in [2.0e-11, 2.2e-11, 2.4e-11, 2.6e-11]:
        for k_anth in [0.20, 0.25, 0.30, 0.35, 0.40]:
            ...
            errs = []
            for t, target in [('CK', 43.3), ('L6D6', 49.4), ('L6D6-N', 49.3),
                              ('VL3D12', 48.2), ('L6D12', 51.8), ('H12D3', 65.1)]:
                if t in results:
                    e = abs((results[t][1] - target) / target * 100)
                    errs.append(e)

            max_err = max(errs) if errs else 100
            all_ok = all(e < 5 for e in errs)
            ...
            if all_ok:
                print(f"找到! base={base:.1e}, V_max={v_max:.1e}, K={k_anth}: max={max_err:.1f}%")

print(f"\n最佳 max_err: {best_max_err:.1f}%")
if best:
    print(f"參數: base={best['base_anth_rate_light']:.1e}, V_max={best['V_max_anth']:.1e}, K={best['K_stress_anth']}")
    print("\n詳細結果:")
    results = test(best)
    for t, target in [('CK', 43.3), ('L6D6', 49.4), ('L6D6-N', 49.3),
                      ('VL3D12', 48.2), ('L6D12', 51.8), ('H12D3', 65.1)]:
        if t in results:
            FW, Anth, Stress = results[t]
            fw_target = TARGETS[t]['FW']
            fw_err = (FW - fw_target) / fw_target * 100
            anth_err = (Anth - target) / target * 100
