...
關鍵洞察：H12D3 只有 3 天照射，但每天 12 小時
- 純 ROS 動態無法區分 6h/天 vs 12h/天，因為 ROS 會被清除
- 需要一個「日內累積能量」變量來記住當天的照射歷史

策略：像 v6.9 一樣追蹤日內累積能量 E_daily，但用 ROS 來驅動
"""
import numpy as np
...
class UVAParamsV8(BaseSunParams):
...
def uva_derivatives_v8(t, state, p, env):
    """ROS v8 - 6 狀態 + 日內能量追蹤（作為 Stress 的一部分）"""
    ...
    # 日內累積能量 (kJ/m²)
    E_daily = hours_exposed_today * 3600 * 22.0 / 1000  # 假設 22 W/m²

    # === v6.9 style depletion factor ===
    E_ref = p.E_ref  # 參考能量 (kJ/m²)
    E_scale = p.E_scale
    k_depletion = p.k_depletion
    depletion_power = p.depletion_power
    sharpness = p.softplus_sharpness

    depletion_raw = softplus((E_daily - E_ref) / E_scale, sharpness)
    depletion_factor = 1.0 + k_depletion * (depletion_raw ** depletion_power)
    ...
    # 損傷 (加入 depletion_factor)
    damage_rate = p.stress_damage_coeff * I_UVA * vulnerability * nonlinear_factor * ros_amplify * circadian_penalty * depletion_factor
...
BASE_PARAMS = {
    ...
    'ros_damage_gain': 1.0,      # 溫和的 ROS 放大
    'K_ros_damage': 10.0,
    # v6.9 style depletion 參數
    'E_ref': 475.2,              # 參考能量 (kJ/m²) ≈ 6h @ 22 W/m²
    'E_scale': 237.6,
    'k_depletion': 54.0,
    'depletion_power': 2.0,
    'softplus_sharpness': 3.0,
}

print("=" * 100)
print("ROS v8 - 結合 ROS + v6.9 日內能量耗竭")
print("=" * 100)

test_all(BASE_PARAMS, "v6.9 style depletion + ROS")

# 調整 ROS 放大
params2 = BASE_PARAMS.copy()
params2['ros_damage_gain'] = 5.0
params2['K_ros_damage'] = 2.0
test_all(params2, "增加 ROS 放大 (gain=5, K=2)")
