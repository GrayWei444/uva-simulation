...
def print_results(results, label):
    ...
    print(f"FW <5%: {sum(1 for e in fw_errs if e < 5)}/6, Anth <10%: {sum(1 for e in anth_errs if e < 10)}/6")

# 測試不同 K_repair_capacity
print("=" * 100)
print("v6.9 參數測試")
print("=" * 100)
...
# 基準 (當前參數)
results = test_params({})
print_results(results, "當前 (K=400, n=2.0)")

# 測試降低 K
for K in [100, 150, 200]:
    results = test_params({'K_repair_capacity': K})
    print_results(results, f"K={K}, n=2.0")

# 測試增加 n
for n in [3.0, 4.0]:
    results = test_params({'K_repair_capacity': 200, 'repair_depletion_power': n})
    print_results(results, f"K=200, n={n}")
