"""測試 ROS 參數 v5 - 非線性累積效應

核心洞察：
L6D6 和 H12D3 有相同的總照射時間 (36h)，但：
- L6D6: 每天 6h，有 18h 恢復
- H12D3: 每天 12h，只有 12h 恢復

問題：純 Michaelis-Menten 動力學無法區分這兩種模式。

新策略：ROS 超線性累積
- 當 ROS 已經較高時，新的 ROS 累積更快（例如：氧化還原失衡）
- 這樣 12h 連續照射會比 2×6h 累積更多 ROS
"""
import numpy as np
...
class UVAParamsV5(BaseSunParams):
    """ROS 版本 v5 - 非線性累積"""
...
def uva_derivatives_v5(t, state, p, env):
    """ROS v5 微分方程 - 非線性 ROS 生成"""
    ...
    # ===== ROS v5: 非線性累積 =====
    # 當 ROS 已經高時，生成效率增加（氧化還原失衡）
    ros_prod_base = p.ros_prod_coeff * I_UVA
    ros_amplification = 1.0 + p.ros_autocatalysis * ROS_mM / (p.K_ros_autocatalysis + ROS_mM + 1e-9)
    ros_prod = ros_prod_base * ros_amplification
...
BASE_PARAMS = {
    ...
    # ROS v5 新參數
    'ros_prod_coeff': 1e-5,
    'ros_autocatalysis': 2.0,       # 新: ROS 自催化係數
    'K_ros_autocatalysis': 1.0,     # 新: 自催化半飽和
    'ros_vmax_apx': 0.0001,
    'ros_vmax_cat': 0.0002,
    'K_ros_apx': 0.074,
    'K_ros_cat': 50.0,
    'ros_passive_decay': 1e-6,
    'ros_damage_gain': 100.0,
    'K_ros_damage': 5.0,
}

print("=" * 120)
print("ROS v5 - 非線性累積策略")
print("說明: ROS 自催化生成，讓長時間照射的 ROS 累積比短時間多更多")
print("=" * 120)

# 測試不同自催化強度
for autocatalysis in [1.0, 2.0, 5.0, 10.0]:
    params = BASE_PARAMS.copy()
    params['ros_autocatalysis'] = autocatalysis
    test_params(params, f"自催化係數 = {autocatalysis}")

# 調整其他參數
print("\n" + "=" * 120)
print("調整自催化 + K_ros_damage")
print("=" * 120)

for K in [2.0, 5.0, 10.0]:
    params = BASE_PARAMS.copy()
    params['ros_autocatalysis'] = 5.0
    params['K_ros_damage'] = K
    test_params(params, f"自催化=5, K_ros_damage={K}")
